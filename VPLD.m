function [VX,VY,P,RESM] = VPLD(VXOTM,VYOTM,PRTM,MUOTM,MUNTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM)
...
for i=2:NIX+1
    VVYI(1,i)=-VYOTM(2,i)*max(-RLOTM(1,i)*FLOTM(1,i)*0.5*VYOTM(2,i),0.0)+0.0;%top boundary
    VVYI(NIY+2,i)=0.0-VYOTM(NIY,i)*min(-RLOTM(NIY+2,i)*FLOTM(NIY+2,i)*0.5*VYOTM(NIY,i),0.0);%bottom boundary
end
...
for i=1:NIY+1
    VDYH(i,1)=2.0*0.25*(MUOTM(i,1)+MUOTM(i,2)+MUOTM(i+1,1)+MUOTM(i+1,2))*(0.5*(FLOTM(i+1,2)+FLOTM(i,2))*VYOTM(i,2)-0.5*(FLOTM(i+1,1)+FLOTM(i,1))*VYOTM(i,1))/(2.0*dx(1));%VYOTM(i,1)=-VYOTM(i,2) for left boundary; [Pa.sec times 1/sec == Pa]
    VDYH(i,NIX+1)=2.0*0.25*(MUOTM(i,NIX+1)+MUOTM(i,NIX+2)+MUOTM(i+1,NIX+1)+MUOTM(i+1,NIX+2))*(0.5*(FLOTM(i+1,NIX+2)+FLOTM(i,NIX+2))*VYOTM(i,NIX+2)-0.5*(FLOTM(i+1,NIX+1)+FLOTM(i,NIX+1))*VYOTM(i,NIX+1))/(2.0*dx(NIX));%VYOTM(i,NIX+2)=-VYOTM(i,NIX+1) for right boundary; [Pa.sec times 1/sec == Pa]
end
...
%DFLP=bicg(A,B,1.0e-12);
%DFLP=gmres(A,B,[],1e-9,1000);
%DFLP=pcg(A,B,1e-9,1000);
%DFLP=bicgstab(A,B,1e-9,1000);
%DFLP=pinv(A)*B;

%     PRG=zeros(NIY*NIX,1);
%     for i=1:NIX
%         PRG(1+(i-1)*NIY:i*NIY)=PRTM(1:NIY,i);
%     end
%     DFLP=lsqr(A,B,1e-9,1000,[],[],PRG);
...
RFVXN(1,:)=-RFVXN(2,:);%top lid but useless
RFVXN(NIY+2,:)=-RFVXN(NIY+1,:);%bottom boundary
%NOTE: Now RFVXN(1:NIY+2,1)=0.0 & RFVXN(1:NIY+2,NIX+1)=0.0
%----------------------------- DRFVX ----------------------------------
...
for i=2:NIX
    VXOTM(1,i)=2.0-VXOTM(2,i);%top lid
