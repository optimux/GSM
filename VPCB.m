function [VX,VY,P,iters] = VPCB(VXOTM,VYOTM,POTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM)
...
%Modified to boundary condition method 2019-12-11
...
for i=1:NIX
    ...
    for i=1:NIX+2
        for j=1:NIY+1
            FRLOTM(j,i)=0.5*(FLOTM(j,i)+FLOTM(j+1,i))*0.5*(RLOTM(j,i)+RLOTM(j+1,i));
            FRLNTM(j,i)=0.5*(FLNTM(j,i)+FLNTM(j+1,i))*0.5*(RLNTM(j,i)+RLNTM(j+1,i));
        end
    end

    FLP=zeros(NIY,NIX);
for i=2:NIX+1
    FLP(1,i-1)=0.25*g*dy(1)*(FRLOTM(1,i)+FRLNTM(1,i));
end
...
%while(errRES>err0RES)
    ...
    for i=2:NIX
    FPDX(2,i)=-2.0*dtb*(FLP(1,i)-FLP(1,i-1))/(dx(i)+dx(i-1));    
    end
