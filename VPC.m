function [VX,VY,P,iters] = VPC(VXOTM,VYOTM,POTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM,dtb)
...
%--------------------------- 15. Old RFVY ---------------------------------
   ...
   %NOTE: you have to set tau_xx|(j+1,k)==tau_xx|(j,k) since partial^2(VX)/partial(x)^2==0.0 at (j+0.5,k) 
    ...
    for i=2:NIX
        for j=1:NIY+2
            FPDX(j,i)=-2.0*dtb*(FLNTM(j,i+1)*(POTM(j,i+1)-P0(j,i+1))-FLNTM(j,i)*(POTM(j,i)-P0(j,i)))/(dx(i)+dx(i-1));%[Pa.sec/m == kg/m^2/sec]
        end
    end
    ...
    for i=1:NIY+2
        FPDX(i,1)=-2.0*dtb*(FLNTM(i,2)*(POTM(i,2)-P0(i,2))-FLNTM(i,1)*(POTM(i,1)-P0(i,1)))/(2.0*dx(1));%[Pa.sec/m == kg/m^2/sec]
        FPDX(i,NIX+1)=-2.0*dtb*(FLNTM(i,NIX+2)*(POTM(i,NIX+2)-P0(i,NIX+2))-FLNTM(i,NIX+1)*(POTM(i,NIX+1)-P0(i,NIX+1)))/(2.0*dx(NIX));%[Pa.sec/m == kg/m^2/sec]
    end
    ...
    for i=1:NIX+2
        for j=2:NIY
            FPDY(j,i)=-2.0*dtb*(FLNTM(j+1,i)*(POTM(j+1,i)-P0(j+1,i))-FLNTM(j,i)*(POTM(j,i)-P0(j,i)))/(dy(j)+dy(j-1));%[Pa.sec/m == kg/m^2/sec]
        end
    end
    
    for i=1:NIX+2
        FPDY(1,i)=-2.0*dtb*(FLNTM(2,i)*(POTM(2,i)-P0(2,i))-FLNTM(1,i)*(POTM(1,i)-P0(1,i)))/(2.0*dy(1));%top boundary  [Pa.sec/m == kg/m^2/sec]
        FPDY(NIY+1,i)=-2.0*dtb*(FLNTM(NIY+2,i)*(POTM(NIY+2,i)-P0(NIY+2,i))-FLNTM(NIY+1,i)*(POTM(NIY+1,i)-P0(NIY+1,i)))/(2.0*dy(NIY));%bottom boundary  [Pa.sec/m == kg/m^2/sec]
    end
    ...
    for i=1:NIX+2
        FRG(1,i)=0.5*(0.5*g*dtb*(FRLOTM(1,i)+FRLNTM(1,i)));%only half cell at top boundary
        FRG(NIY+1,i)=0.5*(0.5*g*dtb*(FRLOTM(NIY+1,i)+FRLNTM(NIY+1,i)));%only half cell at bottom boundary
    end
...
%     end
    ...
    for i=2:NIX+1
        for j=2:NIY+1
            k=(i-2)*NIY+j-1;
            POTM(j,i)=POTM(j,i)+0.8*DFLP(k)/FLNTM(j,i);
        end
    end
    ...
    for i=1:NIX+2
        POTM(1,i)=-POTM(2,i);%top boundary
        POTM(NIY+2,i)=POTM(NIY+1,i)+0.5*g*dy(NIY)*RLNTM(NIY+1,i)+0.5*g*dy(NIY)*RLNTM(NIY+2,i);%bottom boundary
    end
    %-------------------------- New Pressure ------------------------------
...
for i=1:NIY+1
    ...
    %------------------------------ NEW VY ------------------------------------
        [T,FL,CL,dFS,iters]=TFCM(T,FL,FS,KL,KS,CL,CS,VX,VY,FRCPS,FRCS,FK,RSCPS,RLCPL,RS,RL,CpL,TimeInterval);
