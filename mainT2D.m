...
for n=1:NS
    ...
    %To debug, we firstly consider it as a solid state 2D heat conduction problem
    [T,FL,CL,dFS,iters]=TCAL(T,FL,FS,KL,KS,CL,CS,VX,VY,FRCPS,FRCS,FK,RSCPS,RLCPL,RS,RL,CpL,TimeInterval);
    
    T2DPlot(T,t(n+1),n)
    
    %New FL, FS, KP (partition coefficient), CS
    FS=1.0-FL;%FS==0 (always)
    for i=1:NIX+2
        for j=1:NIY+2
            if(CL(j,i)~=CE)%CL stays constant
                KP(j,i)=0.12824+5.699124e-5*CL(j,i)*100.0+3.728777e-5*(CL(j,i)*100.0)^2;%solid/liquid partition coefficient from D-15 [1]; constant
            else
                KP(j,i)=1.0;
            end
            CS(j,i)=CL(j,i)*KP(j,i);%constant
        end
    end
