...
for n=1:NS
    ...
    if(n==1)
        ...
        FindNSE(FSRD(:,:,n-1),FSRD(:,:,n),n);%(dFSRD(:,:,n-1),dFSRD(:,:,n),n);
    end
    ...
    [T,FL,CL,dFS,iters]=TFCCAL(T,FL,FS,KL,KS,CL,CS,VX,VY,FRCPS,FRCS,FK,RSCPS,RLCPL,RS,RL,CpL,TimeInterval);
    ...
    %---------------------------- VX-VY-P ---------------------------------
%     [VX,VY,P,iters]=VPCALM(VX,VY,P,FLO,FL,RLO,RL,RSO,RS,dFS,TimeInterval);
%     fprintf('P-V Loop:%3d iterations!\n',iters);
