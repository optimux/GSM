function [VX,VY,P,iters] = VPCALM(VXOTM,VYOTM,POTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM,dtb)
...
for i=1:NIY
    AX(i,1)=2.0*dy(i)/(dx(1)*(1.0+dtb*UFRKX(i+1,1)));%1st column, [1]
    AX(i,NIX+1)=2.0*dy(i)/(dx(NIX)*(1.0+dtb*UFRKX(i+1,NIX+1)));%last column, [1]
end
...
for i=1:NIX
    AY(1,i)=2.0*dx(i)/(dy(1)*(1.0+dtb*UFRKY(1,i+1)));%1st row, [1]
    AY(NIY+1,i)=2.0*dx(i)/(dy(NIY)*(1.0+dtb*UFRKY(NIY+1,i+1)));%last row, [1]
end
...
%--------------------------- 15. Old RFVY ---------------------------------
    ...
    for i=2:NIY+1
        RFVYI(i,1)=RLNTM(i,1)*FLNTM(i,1)*0.5*(VYOTM(i-1,1)+VYOTM(i,1));%[kg/m^2/sec]
    end
    ...
    for i=1:NIX
        for j=1:NIY+2
            VDXI(j,i)=mu*(VXOTM(j,i+1)*0.5*(FLNTM(j,i+1)+FLNTM(j,i+2))-VXOTM(j,i)*0.5*(FLNTM(j,i)+FLNTM(j,i+1)))/dx(i);%[Pa.sec times 1/sec == Pa]
        end
    end
    ...
    for i=1:NIX+1
        VDXH(1,i)=2.0*mu*(0.5*(FLNTM(2,i)+FLNTM(2,i+1))*VXOTM(2,i)-0.5*(FLNTM(1,i)+FLNTM(1,i+1))*VXOTM(1,i))/dy(1);%[Pa.sec times 1/sec == Pa]
        VDXH(NIY+1,i)=2.0*mu*(0.5*(FLNTM(NIY+2,i)+FLNTM(NIY+2,i+1))*VXOTM(NIY+2,i)-0.5*(FLNTM(NIY+1,i)+FLNTM(NIY+1,i+1))*VXOTM(NIY+1,i))/dy(NIY);%[Pa.sec times 1/sec == Pa]
    end
    ...
    for i=1:NIX+2
        for j=1:NIY
            VDYI(j,i)=mu*(VYOTM(j+1,i)*0.5*(FLNTM(j+2,i)+FLNTM(j+1,i))-VYOTM(j,i)*0.5*(FLNTM(j+1,i)+FLNTM(j,i)))/dy(j);%[Pa.sec times 1/sec == Pa]
        end
    end
    ...
    for i=1:NIY+1
        VDYH(i,1)=2.0*mu*(0.5*(FLNTM(i+1,2)+FLNTM(i,2))*VYOTM(i,2)-0.5*(FLNTM(i+1,1)+FLNTM(i,1))*VYOTM(i,1))/dx(1);%[Pa.sec times 1/sec == Pa]
        VDYH(i,NIX+1)=2.0*mu*(0.5*(FLNTM(i+1,NIX+2)+FLNTM(i,NIX+2))*VYOTM(i,NIX+2)-0.5*(FLNTM(i+1,NIX+1)+FLNTM(i,NIX+1))*VYOTM(i,NIX+1))/dx(NIX);%[Pa.sec times 1/sec == Pa]
    end
    ...
    for i=1:NIY
        VXCONV(i,1)=-2.0*dtb*(VVXI(i+1,1)-0.0)/dx(1)-dtb*(VVXH(i+1,1)-VVXH(i,1))/dy(i);%VVXI have no value at image_1 sites while RFVXI=0.0 at image_1 sites [kg/m^2/sec]
        VXCONV(i,NIX+1)=-2.0*dtb*(0.0-VVXI(i+1,NIX))/dx(NIX)-dtb*(VVXH(i+1,NIX+1)-VVXH(i,NIX+1))/dy(i);%%VVXI have no value at image_2 sites while RFVXI=0.0 at image_2 sites [kg/m^2/sec]
    end
    %--------------------- 7. x-convection part -------------------------------
    ...
    for i=1:NIY
        ...
        VXDIF(i,NIX+1)=2.0*dtb*(0.0-VDXI(i+1,NIX))/dx(NIX)+dtb*(VDXH(i+1,NIX+1)-VDXH(i,NIX+1))/dy(i);%[Pa.sec/m]
    end
    %---------------------- 8. x-diffusion part -------------------------------
    ...
    for i=1:NIX
        ...
        VYDIF(NIY+1,i)=dtb*(VDYH(NIY+1,i+1)-VDYH(NIY+1,i))/dx(i)+2.0*dtb*(0.0-VDYI(NIY,i+1))/dy(NIY);%[Pa.sec/m]
    end
    %---------------------- 10. y-diffusion part ------------------------------
    ...
    for i=1:NIY+2
        FPDX(i,1)=-2.0*dtb*(FLNTM(i,2)*(POTM(i,2)-P0(i,2))-FLNTM(i,1)*(POTM(i,1)-P0(i,1)))/dx(1);%[Pa.sec/m == kg/m^2/sec]
        FPDX(i,NIX+1)=-2.0*dtb*(FLNTM(i,NIX+2)*(POTM(i,NIX+2)-P0(i,NIX+2))-FLNTM(i,NIX+1)*(POTM(i,NIX+1)-P0(i,NIX+1)))/dx(NIX);%[Pa.sec/m == kg/m^2/sec]
    end
    
    %-------------------- 11. x- pressure gradient ----------------------------
    
    %---------------------- 12. y- pressure part ------------------------------
    %     PP=zeros(NIY+1,NIX+2);
    %     PPD=zeros(NIY+1,NIX+2);
    %
    %     for i=1:NIX+2
    %         for j=1:NIY+1
    %             PP(j,i)=POTM(j+1,i)-POTM(j,i);
    %         end
    %     end
    ...
    for i=1:NIX+2
        FPDY(1,i)=-2.0*dtb*(FLNTM(2,i)*(POTM(2,i)-P0(2,i))-FLNTM(1,i)*(POTM(1,i)-P0(1,i)))/dy(1);%[Pa.sec/m == kg/m^2/sec]
        FPDY(NIY+1,i)=-2.0*dtb*(FLNTM(NIY+2,i)*(POTM(NIY+2,i)-P0(NIY+2,i))-FLNTM(NIY+1,i)*(POTM(NIY+1,i)-P0(NIY+1,i)))/dy(NIY);%[Pa.sec/m == kg/m^2/sec]
    end
    ...
    for i=1:NIX
        B(1+(i-1)*NIY:i*NIY)=-0.0*RES(1:NIY,i)/dtb;%[kg/m/sec == Pa.sec]
    end
    ...
    %
    %     C=A(NIY:(NIX-1)*NIY,:);
    %     for i=1:NIY-1:(NIX-1)*NIY
    %         D
    %DFLP(2:NIY-1,2:NIX-1)=-A(2:NIY-1,2:NIX-1)\RES(2:NIY-1,2:NIX-1)/dtb;
    
    %-------------------------- New Pressure ------------------------------
    for i=2:NIX+1
        for j=2:NIY+1
            k=(i-2)*NIY+j-1;
            POTM(j,i)=POTM(j,i)+DFLP(k);
        end
    end
