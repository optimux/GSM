function [VX,VY,P,iters,RESM] = VPMC(VXOTM,VYOTM,POTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM)%C --> constant density
...
for i=2:NIX-1
    for j=2:NIY-1
        ...
        A(k,k-NIY)=-AX(j,i);%[1]
        A(k,k-1)=-AY(j,i);%[1]
        A(k,k)=AX(j,i)+AY(j,i)+AY(j+1,i)+AX(j,i+1)+1.0e-6;%[1]
        A(k,k+1)=-AY(j+1,i);%[1]
...
for i=2:NIY-1
    j=(NIX-1)*NIY+i;
    A(j,j)=AY(i,NIX)+AY(i+1,NIX)+AX(i,NIX)+1.0e-6;%[1]
    A(j,j-1)=-AY(i,NIX);%[1]
...
for i=2:NIX-1
    j=(i-1)*NIY+1;
    A(j,j)=AX(1,i)+AY(2,i)+AX(1,i+1)+1.0e-6;%[1]
    A(j,j-NIY)=-AX(1,i);%[1]
    A(j,j+1)=-AY(2,i);%[1]
...
for i=2:NIX-1
    j=i*NIY;
    A(j,j)=AX(NIY,i)+AY(NIY,i)+AX(NIY,i+1)+1.0e-6;%[1]
...
%--------------------------- 15. Old RFVY ---------------------------------
    ...
    for i=2:NIX+1
        for j=1:NIY+2
            VDXI(j,i)=mu*(VXOTM(j,i)*0.5*(FLOTM(j,i)+FLOTM(j,i+1))-VXOTM(j,i-1)*0.5*(FLOTM(j,i-1)+FLOTM(j,i)))/dx(i-1);%[Pa.sec times 1/sec == Pa]
        end
    end
    
    for i=2:NIY+1
        VDXI(i,1)=mu*(0.0-(-VXOTM(i,2)*0.5*(FLOTM(i,3)+FLOTM(i,2))))/dx(1);%left boundary
        VDXI(i,NIX+2)=mu*(0.5*(FLOTM(i,NIX)+FLOTM(i,NIX+1))*(-VXOTM(i,NIX))-0.0)/dx(NIX);%right boundary
    end
    ...
    for i=1:NIX+1
        VDXH(1,i)=2.0*mu*(0.5*(FLOTM(2,i)+FLOTM(2,i+1))*VXOTM(2,i)-0.5*(FLOTM(1,i)+FLOTM(1,i+1))*VXOTM(1,i))/(2.0*dy(1));%VXOTM(1,i)=-VXOTM(2,i); [Pa.sec times 1/sec == Pa]
        VDXH(NIY+1,i)=2.0*mu*(0.5*(FLOTM(NIY+2,i)+FLOTM(NIY+2,i+1))*VXOTM(NIY+2,i)-0.5*(FLOTM(NIY+1,i)+FLOTM(NIY+1,i+1))*VXOTM(NIY+1,i))/(2.0*dy(NIY));%VXOTM(NIY+1,i)=-VXOTM(NIY+2,i);[Pa.sec times 1/sec == Pa]
    end
    ...
    for i=1:NIX+2
        for j=2:NIY+1
            VDYI(j,i)=mu*(VYOTM(j,i)*0.5*(FLOTM(j+1,i)+FLOTM(j,i))-VYOTM(j-1,i)*0.5*(FLOTM(j,i)+FLOTM(j-1,i)))/dy(j-1);%[Pa.sec times 1/sec == Pa]
        end
    end
    
    for i=2:NIX+1
        VDYI(1,i)=mu*(0.0-0.5*(FLOTM(2,i)+FLOTM(3,i))*(-VYOTM(2,i)))/dy(1);%top boundary
        VDYI(NIY+2,i)=mu*(0.5*(FLOTM(NIY+1,i)+FLOTM(NIY,i))*(-VYOTM(NIY,i))-0.0)/dy(NIY);%bottom boundary
    end
    ...
    for i=1:NIY+1
        VDYH(i,1)=2.0*mu*(0.5*(FLOTM(i+1,2)+FLOTM(i,2))*VYOTM(i,2)-0.5*(FLOTM(i+1,1)+FLOTM(i,1))*VYOTM(i,1))/(2.0*dx(1));%VYOTM(i,1)=VYOTM(i,2) for slip boundary; [Pa.sec times 1/sec == Pa]
        VDYH(i,NIX+1)=2.0*mu*(0.5*(FLOTM(i+1,NIX+2)+FLOTM(i,NIX+2))*VYOTM(i,NIX+2)-0.5*(FLOTM(i+1,NIX+1)+FLOTM(i,NIX+1))*VYOTM(i,NIX+1))/(2.0*dx(NIX));%VYOTM(i,NIX+2)=-VYOTM(i,NIX+1); [Pa.sec times 1/sec == Pa]
    end
    ...
    for i=1:NIY+2
        FPDX(i,1)=-2.0*dtb*(FLOTM(i,2)*(POTM(i,2)-P0(i,2))-FLOTM(i,1)*(POTM(i,1)-P0(i,1)))/(2.0*dx(1));%[Pa.sec/m == kg/m^2/sec]
        FPDX(i,NIX+1)=-2.0*dtb*(FLOTM(i,NIX+2)*(POTM(i,NIX+2)-P0(i,NIX+2))-FLOTM(i,NIX+1)*(POTM(i,NIX+1)-P0(i,NIX+1)))/(2.0*dx(NIX));%[Pa.sec/m == kg/m^2/sec]
    end
    ...
    for i=1:NIX+2
        FPDY(1,i)=-2.0*dtb*(FLOTM(2,i)*(POTM(2,i)-P0(2,i))-FLOTM(1,i)*(POTM(1,i)-P0(1,i)))/(2.0*dy(1));%[Pa.sec/m == kg/m^2/sec]
        FPDY(NIY+1,i)=-2.0*dtb*(FLOTM(NIY+2,i)*(POTM(NIY+2,i)-P0(NIY+2,i))-FLOTM(NIY+1,i)*(POTM(NIY+1,i)-P0(NIY+1,i)))/(2.0*dy(NIY));%[Pa.sec/m == kg/m^2/sec]
    end
...
%     B(1)=0.0;
   ...
   % [DFLP,flags]=PCG(A,B);   
    ...
    for i=2:NIX+1
        for j=2:NIY+1
            k=(i-2)*NIY+j-1;
            POTM(j,i)=(POTM(j,i)*FLOTM(j,i)+DFLP(k))/FLNTM(j,i);
        end
    end
...
%------------------------- MASS BALANCE CHECK -----------------------------
RESM=zeros(NIY+2,NIX+2);%relative residual of mass
mass_check=1.0;%mass check marker
for i=2:NIX+1
    for j=2:NIY+1
     RESM(j,i)=dtb*dy(j-1)*(RFVXN(j,i)-RFVXN(j,i-1))+dtb*dx(i-1)*(RFVYN(j,i)-RFVYN(j-1,i))+0.5*dFSTM(j+1,i+1)*(RSOTM(j+1,i+1)+RSNTM(j+1,i+1))*dx(i)*dy(j)+(FLNTM(j+1,i+1)*RLNTM(j+1,i+1)-FLOTM(j+1,i+1)*RLOTM(j+1,i+1))*dx(i)*dy(j);%absolute mass residual
     RESM(j,i)=RESM(j,i)/(dx(i-1)*dy(j-1)*RL0(j,i));%relative mass residual with respect to initial mass
     if(abs(RESM(j,i))>1.0e-6)
