function [VX,VY,P,iters] = VPCG0(VXOTM,VYOTM,POTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM)
%To solve V-P momentum conservation
%Created 2019-10-20
%Modified for second paper 2020-1-2
...
A((NIX-1)*NIY+1,(NIX-1)*NIY+1)=AX(1,NIX)+AY(2,NIX)+AX(1,NIX+1)+AY(1,NIX);%[1]
...
%     for i=2:3
%         RFVYI(1,i)=2.0*(RLNTM(1,i)+RLNTM(2,i))*0.5*(FLNTM(1,i)+FLNTM(2,i))*0.5*VYOTM(1,i)-RFVYI(2,i);%riser velocity downward
%     end
    ...
    RFVYH(1,3)=0.25*(RLNTM(1,3)+RLNTM(1,4)+RLNTM(2,3)+RLNTM(2,4))*0.25*(FLNTM(1,3)+FLNTM(1,4)+FLNTM(2,3)+FLNTM(2,4))*VYOTM(1,3);
    %----------------------------- 2. RFVY ------------------------------------
...
%     for i=1:3
%         VVYI(1,i)=2.0*(VYOTM(1,i)-VYOTM(2,i))*max(RFVYI(1,i),0.0)+VYOTM(1,i)*min(RFVYI(1,i),0.0);%riser velocity downward
%     end
    ...
    %NOTE: VDYI(1,2:3)=VDYI(2,2:3) since VY at riser inlet is specified, no
    %more informayion is needed!
...
%     VDYH(1,3)=0.0;%riser velocity downward
    ...
    for i=1:NIX
        ...
        %NOTE: VYCONV(1,1:3) is not necessary since VY(1,1:3) is set!
    ...
    for i=1:NIX
        ...
        %NOTE: VYDIF(1,2:3) is not necessary since VY(1,2:3) is set!
    ...
    for i=1:NIX+2
        FRG(1,i)=0.5*g*dtb*(FRLOTM(1,i)+FRLNTM(1,i));%top boundary
        FRG(NIY+1,i)=0.5*g*dtb*(FRLOTM(NIY+1,i)+FRLNTM(NIY+1,i));%bottom boundary
    end
...
%     end
    ...
    for i=1:NIY+1
        RFVYTM(i,1)=-RFVYTM(i,2);
        RFVYTM(i,NIX+2)=-RFVYTM(i,NIX+1);
    end
    %--------------------------- RFVYTM -----------------------------------
    %===================== APPROXIMATION OF RFVXY =========================
    ...
    %Boundary pressure
        ...
        POTM(NIY+2,i)=POTM(NIY+1,i);%bottom boundary
        end
...
%     for i=1:3
%         VYOTM(1,i)=VR;%riser velocity downward
%     end
    %------------------------------ NEW VY ------------------------------------
