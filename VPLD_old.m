function [VX,VY,P,iters] = VPLD(VXOTM,VYOTM,POTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM)
...
%Modified for lid driven cavity flow 2019-11-21
...
%--------------------------- 15. Old RFVY ---------------------------------
    ...
    for i=1:NIX+2
        RFVXI(1,i)=2.0*RLNTM(2,i)*FLNTM(2,i)-RFVXI(2,i);%top VX=0.0 boundary
    ...
    for i=1:NIY+2
        RFVYI(i,1)=-RFVYI(i,2);%left VY=0.0 boundary
    ...
    for i=1:NIY+2
        VVXI(i,1)=VVXI(i,2);%-VXOTM(i,2)*max(RFVXI(i,1),0.0)+VXOTM(i,1)*min(RFVXI(i,1),0.0);%left boundary
        VVXI(i,NIX+2)=VVXI(i,NIX+1);%VXOTM(i,NIX+1)*max(RFVXI(i,NIX+2),0.0)+(-VXOTM(i,NIX))*min(RFVXI(i,NIX+2),0.0);%right boundary
    end
    ...
    for i=1:NIX+2
        VVYI(1,i)=VVYI(2,i);%-VYOTM(2,i)*max(RFVYI(1,i),0.0)+VYOTM(1,i)*min(RFVYI(1,i),0.0);%top boundary
        VVYI(NIY+2,i)=VVYI(NIY+1,i);%VYOTM(NIY+1,i)*max(RFVYI(NIY+2,i),0.0)+(-VYOTM(NIY,i)*min(RFVYI(NIY+2,i),0.0));%bottom boundary
    end
    %Upwind scheme for y-axis mass flow
    ...
    for i=1:NIY+2
        VDXI(i,1)=VDXI(i,2);%==mu*(0.5*(FLNTM(i,1)+FLNTM(i,2))*VXOTM(i,1)-(-VXOTM(i,2)*0.5*(FLNTM(i,3)+FLNTM(i,2))))/dx(1);%left boundary
        VDXI(i,NIX+2)=VDXI(i,NIX+1);%==mu*(0.5*(FLNTM(i,NIX+2)+FLNTM(i,NIX+1))*(-VXOTM(i,NIX))-0.5*(FLNTM(i,NIX+1)+FLNTM(i,NIX+2))*VXOTM(i,NIX+1))/dx(NIX);%right boundary
    end
   %NOTE: you have to set tau_xx|(j+1,k)==tau_xx|(j,k) since partial^2(VX)/partial(x)^2==0.0 at (j+0.5,k) 
    ...
    for i=1:NIX+2
        VDYI(1,i)=VDYI(2,i);%==mu*(VYOTM(1,i)*0.5*(FLNTM(1,i)+FLNTM(2,i))-0.5*(FLNTM(2,i)+FLNTM(3,i))*(-VYOTM(2,i)))/dy(1);%top boundary
        VDYI(NIY+2,i)=VDYI(NIY+1,i);%==mu*(0.5*(FLNTM(NIY+1,i)+FLNTM(NIY,i))*(-VYOTM(NIY,i))-VYOTM(NIY+1,i)*0.5*(FLNTM(NIY+1,i)+FLNTM(NIY+2,i)))/dy(NIY);%bottom boundary
    end
    ...
    for i=1:NIY+1
        VDYH(i,1)=2.0*mu*(0.5*(FLNTM(i+1,2)+FLNTM(i,2))*VYOTM(i,2)-0.5*(FLNTM(i+1,1)+FLNTM(i,1))*VYOTM(i,1))/(2.0*dx(1));%left boundary [Pa.sec times 1/sec == Pa]
    ...
    for i=1:NIX
        ...
        VYCONV(NIY+1,i)=-dtb*(VVYH(NIY+1,i+1)-VVYH(NIY+1,i))/dx(i)-2.0*dtb*(VVYI(NIY+2,i+1)-VVYI(NIY+1,i+1))/(2.0*dy(NIY));%botom boundary [Pa.sec/m]
    end
    ...
    for i=2:NIX
        for j=1:NIY+2
            FPDX(j,i)=-2.0*dtb*(FLNTM(j,i+1)*POTM(j,i+1)-FLNTM(j,i)*POTM(j,i))/(dx(i)+dx(i-1));%[Pa.sec/m == kg/m^2/sec]
        end
    end
    ...
    for i=1:NIY+2
        FPDX(i,1)=-2.0*dtb*(FLNTM(i,2)*POTM(i,2)-FLNTM(i,1)*POTM(i,1))/(2.0*dx(1));%left boundary [Pa.sec/m == kg/m^2/sec]
        FPDX(i,NIX+1)=-2.0*dtb*(FLNTM(i,NIX+2)*POTM(i,NIX+2)-FLNTM(i,NIX+1)*POTM(i,NIX+1))/(2.0*dx(NIX));%right boundary [Pa.sec/m == kg/m^2/sec]
    end
    ...
    for i=1:NIX+2
        for j=2:NIY
            FPDY(j,i)=-2.0*dtb*(FLNTM(j+1,i)*POTM(j+1,i)-FLNTM(j,i)*POTM(j,i))/(dy(j)+dy(j-1));%[Pa.sec/m == kg/m^2/sec]
        end
    end
    
    for i=1:NIX+2
        FPDY(1,i)=-2.0*dtb*(FLNTM(2,i)*POTM(2,i)-FLNTM(1,i)*POTM(1,i))/(2.0*dy(1));%top boundary [Pa.sec/m == kg/m^2/sec]
        FPDY(NIY+1,i)=-2.0*dtb*(FLNTM(NIY+2,i)*POTM(NIY+2,i)-FLNTM(NIY+1,i)*POTM(NIY+1,i))/(2.0*dy(NIY));%bottom boundary [Pa.sec/m == kg/m^2/sec]
    end
...
%     end
    ...
    for i=1:NIX
        for j=1:NIY
            RES(j,i)=dy(j)*(RFVXTM(j+1,i+1)-RFVXTM(j+1,i))+dx(i)*(RFVYTM(j+1,i+1)-RFVYTM(j,i+1))+0.5*dFSTM(j+1,i+1)*(RSOTM(j+1,i+1)+RSNTM(j+1,i+1))*dx(i)*dy(j)/dtb+(FRLNTM(j+1,i+1)-FRLOTM(j+1,i+1))*dx(i)*dy(j)/dtb;%[kg/m/sec == Pa.sec]
        end
    end
    
    for i=2:NIX-1
        RES(1,i)=dy(1)*(RFVXTM(2,i+1)-RFVXTM(2,i))+dx(i)*(RFVYTM(2,i+1)-RFVYTM(1,i+1))+0.5*dFSTM(2,i+1)*(RSOTM(2,i+1)+RSNTM(2,i+1))*dx(i)*dy(1)/dtb+(FRLNTM(2,i+1)-FRLOTM(2,i+1))*dx(i)*dy(1)/dtb;%top boundary
        RES(NIY,i)=dy(NIY)*(RFVXTM(NIY+1,i+1)-RFVXTM(NIY+1,i))+dx(i)*(RFVYTM(NIY+1,i+1)-RFVYTM(NIY,i+1))+0.5*dFSTM(NIY+1,i+1)*(RSOTM(NIY+1,i+1)+RSNTM(NIY+1,i+1))*dx(i)*dy(NIY)/dtb+(FRLNTM(NIY+1,i+1)-FRLOTM(NIY+1,i+1))*dx(i)*dy(NIY)/dtb;%bottom boundary
    end
    
    for i=2:NIY-1
        RES(i,1)=dy(i)*(RFVXTM(i+1,2)-RFVXTM(i+1,1))+dx(1)*(RFVYTM(i+1,2)-RFVYTM(i,2))+0.5*dFSTM(i+1,2)*(RSOTM(i+1,2)+RSNTM(i+1,2))*dx(1)*dy(i)/dtb+(FRLNTM(i+1,2)-FRLOTM(i+1,2))*dx(1)*dy(i)/dtb;%left boundary
        RES(i,NIX)=dy(i)*(RFVXTM(i+1,NIX+1)-RFVXTM(i+1,NIX))+dx(NIX)*(RFVYTM(i+1,NIX+1)-RFVYTM(i,NIX+1))+0.5*dFSTM(i+1,NIX+1)*(RSOTM(i+1,NIX+1)+RSNTM(i+1,NIX+1))*dx(NIX)*dy(i)/dtb+(FRLNTM(i+1,NIX+1)-FRLOTM(i+1,NIX+1))*dx(NIX)*dy(i)/dtb;%right boundary
    end
    
    RES(1,1)=dy(1)*(RFVXTM(2,2)-RFVXTM(2,1))+dx(1)*(RFVYTM(2,2)-RFVYTM(1,2))+0.5*dFSTM(2,2)*(RSOTM(2,2)+RSNTM(2,2))*dx(1)*dy(1)/dtb+(FRLNTM(2,2)-FRLOTM(2,2))*dx(1)*dy(1)/dtb;
    RES(1,NIX)=dy(1)*(RFVXTM(2,NIX+1)-RFVXTM(2,NIX))+dx(NIX)*(RFVYTM(2,NIX+1)-RFVYTM(1,NIX+1))+0.5*dFSTM(2,NIX+1)*(RSOTM(2,NIX+1)+RSNTM(2,NIX+1))*dx(NIX)*dy(1)/dtb+(FRLNTM(2,NIX+1)-FRLOTM(2,NIX+1))*dx(NIX)*dy(1)/dtb;
    RES(NIY,1)=dy(NIY)*(RFVXTM(NIY+1,2)-RFVXTM(NIY+1,1))+dx(1)*(RFVYTM(NIY+1,2)-RFVYTM(NIY,2))+0.5*dFSTM(NIY+1,2)*(RSOTM(NIY+1,2)+RSNTM(NIY+1,2))*dx(1)*dy(NIY)/dtb+(FRLNTM(NIY+1,2)-FRLOTM(NIY+1,2))*dx(1)*dy(NIY)/dtb;
    RES(NIY,NIX)=dy(NIY)*(RFVXTM(NIY+1,NIX+1)-RFVXTM(NIY+1,NIX))+dx(NIX)*(RFVYTM(NIY+1,NIX+1)-RFVYTM(NIY,NIX+1))+0.5*dFSTM(NIY+1,NIX+1)*(RSOTM(NIY+1,NIX+1)+RSNTM(NIY+1,NIX+1))*dx(NIX)*dy(NIY)/dtb+(FRLNTM(NIY+1,NIX+1)-FRLOTM(NIY+1,NIX+1))*dx(NIX)*dy(NIY)/dtb;
    ...
    for i=2:NIX+1
        for j=2:NIY+1
            k=(i-2)*NIY+j-1;
            POTM(j,i)=POTM(j,i)+0.75*DFLP(k)/FLNTM(j,i);%0.8 from John D. Anderson, JR on page 209
        end
    end
    
    for i=2:NIY+2
        POTM(i,1)=POTM(i,2);%left boundary
        POTM(i,NIX+2)=POTM(i,NIX+1);%right boundary
    end
    for i=1:NIX+2
        POTM(1,i)=POTM(2,i);%-POTM(2,i);%top boundary
        POTM(NIY+2,i)=POTM(NIY+1,i);%POTM(NIY+1,i)+0.5*g*dy(NIY)*RLNTM(NIY+1,i)+0.5*g*dy(NIY)*RLNTM(NIY+2,i);%bottom boundary
    end
    %-------------------------- New Pressure ------------------------------
...
for i=2:NIX
    VXOTM(1,i)=2.0-VXOTM(2,i);%top boundary
...
for i=2:NIY
    VYOTM(i,1)=-VYOTM(i,2);%left boundary
