function  TimeInterval= TSTEPG(FRCPSTM,FLTM,RLTM,CPLTM,KLTM,FKTM,VXTM,VYTM,step)
...
for i=2:NIX-1
    FKD(1,i)=2.0*KX(2,i)/((dx(i-1)+dx(i))*dx(i))+2.0*KX(2,i+1)/((dx(i)+dx(i+1))*dx(i))+2.0*KY(1,i+1)/((dy(1)+dy(1))*dy(1))+2.0*KY(2,i+1)/((dy(1)+dy(2))*dy(1));%set dy(0)==dy(1); top boundary [W/m^3/K]
    FKD(NIY,i)=2.0*KX(NIY+1,i)/((dx(i-1)+dx(i))*dx(i))+2.0*KX(NIY+1,i+1)/((dx(i)+dx(i+1))*dx(i))+2.0*KY(NIY,i+1)/((dy(NIY-1)+dy(NIY))*dy(NIY))+2.0*KY(NIY+1,i+1)/((dy(NIY)+dy(NIY))*dy(NIY));%set dy(NIY+1)==dy(NIY); bottom boundary [W/m^3/K]
end

for i=2:NIY-1
    FKD(i,1)=2.0*KX(i+1,1)/((dx(1)+dx(1))*dx(1))+2.0*KX(i+1,2)/((dx(1)+dx(2))*dx(1))+2.0*KY(i,2)/((dy(i-1)+dy(i))*dy(i))+2.0*KY(i+1,2)/((dy(i)+dy(i+1))*dy(i));%set dx(0)==dx(1); left boundary [W/m^3/K]
    FKD(i,NIX)=2.0*KX(i+1,NIX)/((dx(NIX-1)+dx(NIX))*dx(NIX))+2.0*KX(i+1,NIX+1)/((dx(NIX)+dx(NIX))*dx(NIX))+2.0*KY(i,NIX+1)/((dy(i-1)+dy(i))*dy(i))+2.0*KY(i+1,NIX+1)/((dy(i)+dy(i+1))*dy(i));%set dx(NIX+1)==dx(NIX); right boundary [W/m^3/K]
end

FKD(1,1)=2.0*KX(2,1)/((dx(1)+dx(1))*dx(1))+2.0*KX(2,2)/((dx(2)+dx(1))*dx(1))+2.0*KY(1,2)/((dy(1)+dy(1))*dy(1))+2.0*KY(2,2)/((dy(1)+dy(2))*dy(1));%set dy(0)==dy(1) dx(0)==dx(1);top-left point
FKD(NIY,1)=2.0*KX(NIY+1,1)/((dx(1)+dx(1))*dx(1))+2.0*KX(NIY+1,2)/((dx(2)+dx(1))*dx(1))+2.0*KY(NIY,2)/((dy(NIY-1)+dy(NIY))*dy(NIY))+2.0*KY(NIY+1,2)/((dy(NIY)+dy(NIY))*dy(NIY));%set dy(NIY+1)==dy(NIY) dx(0)==dx(1); bottom-left point
FKD(1,NIX)=2.0*KX(2,NIX)/((dx(NIX-1)+dx(NIX))*dx(NIX))+2.0*KX(2,NIX+1)/((dx(NIX)+dx(NIX))*dx(NIX))+2.0*KY(1,NIX+1)/((dy(1)+dy(1))*dy(1))+2.0*KY(2,NIX+1)/((dy(2)+dy(1))*dy(1));%set dy(0)==dy(1) dx(NIX+1)==dx(NIX); right-top point
FKD(NIY,NIX)=2.0*KX(NIY+1,NIX)/((dx(NIX-1)+dx(NIX))*dx(NIX))+2.0*KX(NIY+1,NIX+1)/((dx(NIX)+dx(NIX))*dx(NIX))+2.0*KY(NIY,NIX+1)/((dy(NIY-1)+dy(NIY))*dy(NIY))+2.0*KY(NIY+1,NIX+1)/((dy(NIY)+dy(NIY))*dy(NIY));%set dx(NIX+1)==dx(NIX) dy(NIY+1)==dy(NIY); right-bottom point
