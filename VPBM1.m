...
%To identify solid or liquid in cell: 1=solid, 0=mixture
SOL=zeros(NIY+2,NIX+2);
for i=1:NIX+2
    for j=1:NIY+2
        if(FLNTM(j,i)<=0.0)
            SOL(j,i)=1.0;
...
for i=1:NIX+2
    for j=1:NIY+1
        ...
        %helps when FL=0.0; but this addition is discarded.
        UFRKY(j,i)=0.5*(MUNTM(j,i)+MUNTM(j+1,i))*0.5*(FLNTM(j,i)+FLNTM(j+1,i))/(0.5*(RLNTM(j,i)+RLNTM(j+1,i))*0.5*(KFLTM(j,i)+KFLTM(j+1,i)+1.0e-16));%[1/sec]
    end
end
...
for i=1:NIX+1
    for j=1:NIY+2
        %IMPORTANT NOTE: 1.0e-16 is added to prevent devision of 0, this
        %helps when FL=0.0
        UFRKX(j,i)=0.5*(MUNTM(j,i)+MUNTM(j,i+1))*0.5*(FLNTM(j,i)+FLNTM(j,i+1))/(0.5*(RLNTM(j,i)+RLNTM(j,i+1))*0.5*(KFLTM(j,i)+KFLTM(j,i+1)+1.0e-16));%[1/sec]
    end
end
    ...
    for j=2:NIY+1
       ...
       RESM(j,i)=dtb*dy(j-1)*(RFVXN(j,i)-RFVXN(j,i-1))+dtb*dx(i-1)*(RFVYN(j,i)-RFVYN(j-1,i))+0.5*dFSTM(j,i)*(RLOTM(j,i)+RLNTM(j,i))*dx(i-1)*dy(j-1)+(FLNTM(j,i)*RLNTM(j,i)-FLOTM(j,i)*RLOTM(j,i))*dx(i-1)*dy(j-1);%absolute mass residual [kg/sec]
        ...
        if(abs(RESMR(j,i))>1.0e-9)
            fprintf('Mass check: (%2d,%2d) not balanced! %E\n',j-1,i-1,RESMR(j,i));
            mass_check=-1.0;
        end
