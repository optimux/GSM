function [VX,VY,P,RESM] = VPBM(VXOTM,VYOTM,PRTM,MUOTM,MUNTM,FLOTM,FLNTM,RLOTM,RLNTM,RSOTM,RSNTM,dFSTM,RLOGTM,RLNGTM)
...
VErr=1.0e-16;%shreshold of error for determining VX, VY=0.0; in Xu's paper, the minimum veleocity is ~ 10^-5 m/sec
%NOTE: we have set 1.0e-9, 1.0e-6, 1.0e-5 for VErr to eliminate bulge at upper
%part, but it turned out useless, the bulge seems to be an intrinsic feature

% %To identify solid or liquid in cell: 0=solid, 1=mixture
% SOL=zeros(NIY,NIX);
% %how many mixture cells are there
% LiquidMarker=0;
% %Where is new righ boundary, set initial right boundary
% rightb=NIX;
% 
% for i=1:NIX
%     for j=1:NIY
%         if(FLNTM(j+1,i+1)>FLErr)%mixture
%             SOL(j,i)=1.0;
%             LiquidMarker=LiquidMarker+1;
%             rightb=i;
%         end
%     end
% end
...
for i=1:NIX+2
    for j=1:NIY+2
        if(FLNTM(j,i)>=1.0/3.0)
            KFLTM(j,i)=2.6e-5*(1.923e-2*FLNTM(j,i)^2+(4.0+3.0*FSNTM(j,i)-3.0*real(sqrt(FSNTM(j,i)*(8.0-3.0*FSNTM(j,i)))))/FSNTM(j,i));%real: some sqrt generate complex numbers; [mm^2]
        else
...
%=========================== [A] RESHAPE ===============================
%NOTE: after some cells are completely solidified, we remove them from
%system, i.e., we do not solve them

%M: Mixture
% AM=zeros(LiquidMarker,LiquidMarker);
% solidcolumn=NIY-int32(sum(SOL(:,1)));%how many cells are solid in first column
% liquidmark=1;
%
% %First column
% for i=2:NIY
%         if(FLNTM(i+1,2)>FLErr)%solid+liquid
%             AM(i,i-1)=A(i,i-1);
%             AM(i,i)=A(i,i);
%             AM(i,i+1)=A(i,i+1);
%             AM(i,i+NIY)=A(i,i+NIY);
%             liquidmark=liquidmark+1;
%         end
% end
% AM(1,1)=A(1,1);
% solidmark=NIY-liquidmark;
%
% %Interoir cells
% for i=2:rightb-1
%     for j=1:NIY
%         k=(i-1)*NIY+j;
%         if(FLNTM(j+1,i+1)>FLErr)%solid+liquid
%                         AM(k-solidmark,k-solidmark-NIY)=A(k,k-NIY);
%             AM(k-solidmark,k-solidmark-1)=A(k,k-1);
%             AM(k-solidmark,k-solidmark)=A(k,k);
%             AM(k-solidmark,k-solidmark+1)=A(k,k+1);
%             AM(k-solidmark,k-solidmark+NIY)=A(k,k+NIY);
%         else%complete solid
%             solidmark=solidmark+1;
%         end
%     end
% end
%
% %Last column: rightboundary
% k=(rightb-1)*NIY;
% for i=1:NIY-1
%     if(FLNTM(i+1,rightb+1)>FLErr)
%                 AM(k+i-solidmark,k+i-solidmark-NIY)=A(k+i,k+i-NIY);
%                 AM(k+i-solidmark,k+i-solidmark-1)=A(k+i,k+i-1);
%         AM(k+i-solidmark,k+i-solidmark)=A(k+i,k+i);
%          AM(k+i-solidmark,k+i-solidmark+1)=A(k+i,k+i+1);
%     else
%         solidmark=solidmark+1;
%     end
...
for i=2:NIX+1
    for j=1:NIY+2
        VDXI(j,i)=MUOTM(j,i)*FLOTM(j,i)*(VXOTM(j,i)-VXOTM(j,i-1))/dx(i-1);%[Pa.sec times 1/sec == Pa]
    end
end

for i=2:NIY+1
    VDXI(i,1)=MUOTM(i,1)*FLOTM(i,1)*(0.0-(-VXOTM(i,2)))/dx(1);%left boundary
    VDXI(i,NIX+2)=MUOTM(i,NIX+2)*FLOTM(i,NIX+2)*((-VXOTM(i,NIX))-0.0)/dx(NIX);%right boundary
end
...
for i=1:NIX+1
    for j=2:NIY
        %VXOTM(j+1,i)~=0.0 and VXOTM(j,i)~=0.0
        %VXOTM(j+1,i)=0.0 and VXOTM(j,i)=0.0
        VDXH(j,i)=2.0*0.25*(MUOTM(j,i)+MUOTM(j,i+1)+MUOTM(j+1,i)+MUOTM(j+1,i+1))*0.25*(FLOTM(j+1,i)+FLOTM(j+1,i+1)+FLOTM(j,i)+FLOTM(j,i+1))*(VXOTM(j+1,i)-VXOTM(j,i))/(dy(j-1)+dy(j));%[Pa.sec times 1/sec == Pa]
        ...
        if(UDL1&&UDL2)
            %VXOTM(j+1,i)=0.0, VXOTM(j,i)~=0.0 and FLOTM(j+1,i+1)=0.0 or FLOTM(j+1,i)=0.0; set VXOTM(j+1,i)=-VXOTM(j,i)
            VDXH(j,i)=2.0*0.25*(MUOTM(j,i)+MUOTM(j,i+1)+MUOTM(j+1,i)+MUOTM(j+1,i+1))*0.25*(FLOTM(j+1,i)+FLOTM(j+1,i+1)+FLOTM(j,i)+FLOTM(j,i+1))*(-VXOTM(j,i)-VXOTM(j,i))/(dy(j-1)+dy(j));%[Pa.sec times 1/sec == Pa]
        end
        ...
        if(UDL3&&UDL4)
            %VXOTM(j+1,i)~=0.0, VXOTM(j,i)=0.0 and FLOTM(j,i+1)=0.0 or FLOTM(j,i)=0.0; set VXOTM(j,i)=-VXOTM(j+1,i)
            VDXH(j,i)=2.0*0.25*(MUOTM(j,i)+MUOTM(j,i+1)+MUOTM(j+1,i)+MUOTM(j+1,i+1))*0.25*(FLOTM(j+1,i)+FLOTM(j+1,i+1)+FLOTM(j,i)+FLOTM(j,i+1))*(-VXOTM(j,i)-(-VXOTM(j+1,i)))/(dy(j-1)+dy(j));%[Pa.sec times 1/sec == Pa]
        end
...
for i=1:NIX+1
    VDXH(1,i)=2.0*0.25*(MUOTM(1,i)+MUOTM(1,i+1)+MUOTM(2,i)+MUOTM(2,i+1))*0.25*(FLOTM(2,i)+FLOTM(2,i+1)+FLOTM(1,i)+FLOTM(1,i+1))*(VXOTM(2,i)-VXOTM(1,i))/(2.0*dy(1));%VXOTM(1,i)=-VXOTM(2,i); [Pa.sec times 1/sec == Pa]
    VDXH(NIY+1,i)=2.0*0.25*(MUOTM(NIY+1,i)+MUOTM(NIY+1,i+1)+MUOTM(NIY+2,i)+MUOTM(NIY+2,i+1))*0.25*(FLOTM(NIY+2,i)+FLOTM(NIY+2,i+1)+FLOTM(NIY+1,i)+FLOTM(NIY+1,i+1))*(VXOTM(NIY+2,i)-VXOTM(NIY+1,i))/(2.0*dy(NIY));%VXOTM(NIY+1,i)=-VXOTM(NIY+2,i);[Pa.sec times 1/sec == Pa]
end
...
for i=1:NIX+2
    for j=2:NIY+1
        VDYI(j,i)=MUOTM(j,i)*FLOTM(j,i)*(VYOTM(j,i)-VYOTM(j-1,i))/dy(j-1);%[Pa.sec times 1/sec == Pa]
    end
end

for i=2:NIX+1
    VDYI(1,i)=MUOTM(1,i)*FLOTM(1,i)*(0.0-(-VYOTM(2,i)))/dy(1);%top boundary
    VDYI(NIY+2,i)=MUOTM(NIY+2,i)*FLOTM(NIY+2,i)*((-VYOTM(NIY,i))-0.0)/dy(NIY);%bottom boundary
end
...
for i=2:NIX
    for j=1:NIY+1
        %VYOTM(j,i)~=0.0 and VYOTM(j,i+1)~=0.0
        %VYOTM(j,i)=0.0 and VYOTM(j,i+1)=0.0
        VDYH(j,i)=2.0*0.25*(MUOTM(j,i)+MUOTM(j,i+1)+MUOTM(j+1,i)+MUOTM(j+1,i+1))*0.25*(FLOTM(j+1,i+1)+FLOTM(j,i+1)+FLOTM(j+1,i)+FLOTM(j,i))*(VYOTM(j,i+1)-VYOTM(j,i))/(dx(i-1)+dx(i));%[Pa.sec times 1/sec == Pa]
        ...
        if(LRL1&&LRL2)
            %VYOTM(j,i)=0.0 and VYOTM(j,i+1)~=0.0 and FLOTM(j,i)=0.0 or FLOTM(j+1,i)=0.0; set VYOTM(j,i)=-VYOTM(j,i+1)
            VDYH(j,i)=2.0*0.25*(MUOTM(j,i)+MUOTM(j,i+1)+MUOTM(j+1,i)+MUOTM(j+1,i+1))*0.25*(FLOTM(j+1,i+1)+FLOTM(j,i+1)+FLOTM(j+1,i)+FLOTM(j,i))*(VYOTM(j,i+1)-(-VYOTM(j,i+1)))/(dx(i-1)+dx(i));%[Pa.sec times 1/sec == Pa]
        end
        ...
        if(LRL3&&LRL4)
            %VYOTM(j,i)~=0.0 and VYOTM(j,i+1)=0.0; set VYOTM(j,i+1)=-VYOTM(j,i)
            VDYH(j,i)=2.0*0.25*(MUOTM(j,i)+MUOTM(j,i+1)+MUOTM(j+1,i)+MUOTM(j+1,i+1))*0.25*(FLOTM(j+1,i+1)+FLOTM(j,i+1)+FLOTM(j+1,i)+FLOTM(j,i))*((-VYOTM(j,i))-(-VYOTM(j,i+1)))/(dx(i-1)+dx(i));%[Pa.sec times 1/sec == Pa]
        end
...
for i=1:NIY+1
    VDYH(i,1)=2.0*0.25*(MUOTM(i,1)+MUOTM(i,2)+MUOTM(i+1,1)+MUOTM(i+1,2))*0.25*(FLOTM(i+1,2)+FLOTM(i,2)+FLOTM(i+1,1)+FLOTM(i,1))*(VYOTM(i,2)-VYOTM(i,1))/(2.0*dx(1));%VYOTM(i,1)=VYOTM(i,2) for slip boundary; [Pa.sec times 1/sec == Pa]
    VDYH(i,NIX+1)=2.0*0.25*(MUOTM(i,NIX+1)+MUOTM(i,NIX+2)+MUOTM(i+1,NIX+1)+MUOTM(i+1,NIX+2))*0.25*(FLOTM(i+1,NIX+2)+FLOTM(i,NIX+2)+FLOTM(i+1,NIX+1)+FLOTM(i,NIX+1))*(VYOTM(i,NIX+2)-VYOTM(i,NIX+1))/(2.0*dx(NIX));%VYOTM(i,NIX+2)=-VYOTM(i,NIX+1); [Pa.sec times 1/sec == Pa]
end
...
% %Tranform into Mixture domain: M=Marker
% BM=zeros(LiquidMarker,1);
% solidmark=0;
% for i=1:NIX
%     for j=1:NIY
%         k=(i-1)*NIY+j;
