...
%Modified from Daming.m created on 2019-9-23
...
%NOTE FOR CORRECTION: (1,NIX+2) should not be adiabatic, to allow heat loss
...
%Xu1991, Numerical Heat Transfer
% W=160.0/1000.0;%ingot width [m]
% H=200.0/1000.0;%ingot height [m]
% WM=360.0/1000.0;%mould width [m]
% HM=320.0/1000.0;%mould height [m]

NIX=10;%number of unit in ingot, x-axis
NIY=20;%number of unit in ingot, y-axis
...
%TM=26.0+273.15;%Initial T of mould [K] ==> PhD thesis
TM=25.0+273.15;%Initial T of mould [K] ==> 1991 Numerical Heat Transfer
TE=548.0+273.15;%eutectic T [K]
FSE=zeros(NIY+2,NIX+2);%solid fraction when eutectic, we round 0.9281 to 0.93 at first
FSELOG=ones(NIY+2,NIX+2);

CL0=0.045;%initial liquid alloy composition as Cu (==4.5 wt%) [1]; It's mass fraction (zhi-liang-bai-fen-nong-du), 4.5 g/100 g=4.5 wt.%=0.045. Thus CL has unit of [1]
...
lambda=0.5;%under-relaxation coefficient
bsq=1.0;%Boussinesq approxiamation: 0=yes, 1=no

NS=100;%579;700;%1315?%total steps
%579: x=10,y=20,t=33.02 sec
%13: x=8, y=12, t=2.21 sec
...
VSX.OL=zeros(NIY+2,NIX+1);
VSX.OPX=zeros(NIY+2,NIX+1);
VSX.CPX=zeros(NIY+2,NIX+1);
VSX.PL=zeros(NIY+2,NIX+1);
VSX.ILM=zeros(NIY+2,NIX+1);
VSY.OL=zeros(NIY+1,NIX+2);
VSY.OPX=zeros(NIY+1,NIX+2);
VSY.CPX=zeros(NIY+1,NIX+2);
VSY.PL=zeros(NIY+1,NIX+2);
VSY.ILM=zeros(NIY+1,NIX+2);

PA=zeros(NIY+2,NIX+2);%pressure in liquid [Pa]; only [2:NIY+1,2:NIX+1] valid, outmost layer for plot
...
RSB=RS;%density for Boussinesq approximation
...
RL0=RL;%reference density at t=0 sec
RLB=RL;%density for Boussinesq approximation
...
for n=1:NS
    ...
    [T,FL,CL,dFS,RESE]=TFCM(T,FL,FS,KL,CL,CS,VX,VY,FRCPS,FK,RSCPS,RLCPL,RS,RL,CpL);%TFCM is original subroutine
    %   T(3,9)-T(2,9)
    %   if(T(3,9)-T(2,9)>=1.0e-16)
    %                 n
    %         break;
    %     end
    %--------------------------- T-FS-CL ----------------------------------
    ...
    CSM=100.0*FRCS./FR;%old mean solid concentration [wt %]
    ...
    %---------------------------- VX-VY-P ---------------------------------
    %    [VX,VY,P,iters]=VPCB(VX,VY,P,FLO,FL,RLO,RL,RSO,RS,dFS);
    ...
    [VX,VY,PR,RESM]=VPBM(VX,VY,PR,MUO,MU,FLO,FL,RLB,RLB,RSB,RSB,dFS,RLO,RL);
    %[VX,VY,PR,RESM]=VPBM(VX,VY,PR,FLO,FL,RLO,RL,RSO,RS,dFS,RLO,RL);
...
% save('FL.mat','-mat','FL');
% save('FS.mat','-mat','FS');
% save('dFS.mat','-mat','dFS');
% save('VX.mat','-mat','VX');
% save('VY.mat','-mat','VY');
% save('PR.mat','-mat','PR');
% save('FRCPS.mat','-mat','FRCPS');
% save('FRCS.mat','-mat','FRCS');
% save('FK.mat','-mat','FK');
% save('FR.mat','-mat','FR');
% save('CpL.mat','-mat','CpL');
% save('CpS.mat','-mat','CpS');
% save('CS.mat','-mat','CS');
% save('KS.mat','-mat','KS');
% save('KL.mat','-mat','KL');
% save('CL.mat','-mat','CL');
% save('MU.mat','-mat','MU');
% save('MUO.mat','-mat','MUO');
% save('FLO.mat','-mat','FLO');
% save('RLO.mat','-mat','RLO');

% save to vtk file for ParaView
% VTKfile='matlab_export.vtk';
% Project='Xudaming';
% property='pressure';
% z=0.0;
% VTK3D(VTKfile,Project,'ascii','structured_grid',property,PR,x,y,z,'PRECISION','4');
% Mat2VTK('Presure.vtk',PR,'ascii');

PlotTFC(T,FS,CL,CSM,RM,VX,VY,PR,t(n+1),n);
OriginPlot(T,FS,VX,VY,VSX,VSY);
%close(aviobj);
T(2,2)